...
task_name = 'regional'
path = '/Users/kzhao46/Dropbox/628-2/data/res_by_name/newdata/Regional/'
...
def split_JJ_all(x):
    # print (x)
    JJ = ''
    for item in x['food_near']:
        # print (item)
        JJ = JJ + ' ' + item[1]
    for item in x['service_near']:
        # print (item)
        JJ = JJ + ' ' + item[1]
    return JJ
...
data_df['JJ'] = data_df.apply(split_JJ_all, axis=1)
data_df['RB'] = data_df.apply(split_RB_all, axis=1)
tag = 'food_near'
